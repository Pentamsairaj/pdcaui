...
$(() => {
    ...
    const CLIENT_EDIT_PROFILE_URL = APIS.clientEditProfile;
    const CLIENT_CHANGE_PASSWORD_URL = APIS.clientChangePassword;
    ...
    const getAdminProfile = () => {
        ...
        $.get(`${CLIENT_PROFILE_URL}?id=${CLIENT_AUTH}`).done((ReqData) => {
            const data = JSON.parse(ReqData.data)
            $("#adminName").html(data.NameAcc);
            $("#adminProfile").html(data.EmailidAcc);
            $("#adminPhone").html(data.PhoneAcc);
            if (data.email == null) {
            ...
            $("#editAdminName").val(data.NameAcc);
            $("#editAdminPhone").val(data.PhoneAcc);
            $("#editAdminEmail").val(data.email);
            $("#drivelink").html(data.drivelink);
    ...
    $("#saveAdminEditInfo").click(function () {
        ...
        $.post(CLIENT_EDIT_PROFILE_URL, { "id": CLIENT_AUTH, "phonenumber": adminPhone, "email": adminEmail, "NameAcc": adminName })
            .done((data) => {
    ...
    $("#saveAdimnResetPassword").click(() => {
        ...
        if (newPassord === VerifyNewPassword) {
            ...
            $.post(CLIENT_CHANGE_PASSWORD_URL, { "id": ADMIN_AUTH, "password": newPassord })
                .done((data) => {
                    if (data.responsecode == 1) {
    ...
    //                     }
    //                     else if (data.responsecode == 2) {
    //                         PAGE_REDIRECTION("/Admin/Login.html");
    //                     }
    //                     else if (data.responsecode == 6) {
    //                         ERROR_MESSAGE("Sorry, Something went wrong");
    //                     }
